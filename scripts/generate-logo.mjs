...
const COUNTRY_COLOURS = new Map([
  // Northern Africa
  ...["012", "434", "504", "729", "788", "818", "732"].map((id) => [
    id,
    C.northern_africa,
  ]),
  ...
  // Southern Asia
  ...["004", "050", "064", "144", "356", "462", "524", "586"].map((id) => [
    id,
    C.southern_asia,
  ]),
  ...
  // Eastern Asia
  ...["156", "158", "392", "408", "410", "496"].map((id) => [
    id,
    C.eastern_asia,
  ]),
...
function buildIco(pngBuffers) {
  ...
  pos += 2 // type: 1 = ICO
  out.writeUInt16LE(pngBuffers.length, pos)
  pos += 2 // image count
  ...
  for (const { buf, offset: imgOffset } of entries) {
    ...
    out.writeUInt32LE(buf.length, pos)
    pos += 4 // image data size
    out.writeUInt32LE(imgOffset, pos)
    pos += 4 // offset in file
  }
...
const pngs = await Promise.all(
  [16, 32, 48].map((size) =>
    sharp(Buffer.from(iconSvg)).resize(size, size).png().toBuffer(),
  ),
)
